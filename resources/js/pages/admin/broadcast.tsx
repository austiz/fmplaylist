...
export default function Broadcast({
    ...
    const filteredSoundBytes = soundBytes.filter((s) =>
        `${s.title} ${s.category}`
            .toLowerCase()
            .includes(soundByteSearch.toLowerCase()),
    );
                ...
                <div className="flex items-start justify-between gap-6">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                            Immediately cuts the current song, clears the queue,
                            and plays <code>announcement.wav</code> from the Pi.
                        </p>
                    </div>
                    <Button
                        type="button"
                        ...
                        >
                            {MODES.map((m) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                        modeForm.setData('broadcast_mode', m.id)
                                    }
                            ...
                            {currentMode === 'phone_stream' && (
                                <div className="border border-violet-500/20 bg-violet-500/5 p-4 text-sm">
                                    <p className="font-semibold text-violet-300">
                                        Stream URL for Larix Broadcaster
                                        (iOS/Android):
                                    </p>
                                    ...
                                    <p className="mt-2 text-xs text-muted-foreground">
                                        Larix → Settings → Connections → Add →
                                        RTMP → paste URL above → Stream Name:{' '}
                                        <code>live</code>
                                    </p>
                                    {!pi.ip && (
                                        <p className="mt-2 text-xs text-yellow-500">
                                            ⚠ Pi IP unknown — Pi must connect
                                            first
                                        </p>
                                    )}
                                </div>
                            ...
                            {currentMode === 'usb_input' && (
                                <div className="space-y-1 p-1">
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        USB audio interface is usually{' '}
                                        <code>hw:1,0</code>. Run{' '}
                                        <code>arecord -l</code> on Pi to list
                                        devices.
                                    </p>
                                </div>
                            )}
                            ...
                            {currentMode === 'custom_stream' && (
                                <div className="space-y-1 p-1">
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        RTMP, HLS, Icecast, or any URL ffmpeg
                                        can read.
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            >
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        setSoundByteSearch(e.target.value)
                                    }
                                />
                                <div className="max-h-40 divide-y divide-border overflow-y-auto border border-border">
                                    {filteredSoundBytes
                ...
                <div>
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            <Label>
                                Program Service (PS) — station name on radio
                                display
                            </Label>
                            <Input
                            ...
                            />
                            <p className="text-xs text-muted-foreground">
                                Max 8 chars. Shown as the station name on every
                                car radio. Leave blank to use the callsign from
                                Settings.
                            </p>
                            {rdsForm.data.rds_ps && (
                                <p className="font-mono text-xs text-muted-foreground">
                                    ...
                                    <span className="text-foreground">
                                        &ldquo;{rdsForm.data.rds_ps.padEnd(8)}
                                        &rdquo;
                                    </span>
                                </p>
                            )}
                        ...
                        <div className="space-y-2">
                            <Label>
                                RadioText (RT) — scrolling message on radio
                                display
                            </Label>
                            ...
                            {rdsForm.data.rds_rt_mode === 'custom' && (
                                <div className="space-y-1">
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        {rdsForm.data.rds_rt.length}/64 chars.
                                        Scrolls across the radio display on all
                                        RDS radios.
                                    </p>
                                </div>
                            )}
                            {rdsForm.data.rds_rt_mode === 'auto' && (
                                <p className="text-xs text-muted-foreground">
                                    RadioText shows the current song title and
                        ...
                        <p className="text-xs text-muted-foreground/50">
                            Changes apply to Pi within 30 seconds (next
                            heartbeat).
                        </p>
                    </form>
