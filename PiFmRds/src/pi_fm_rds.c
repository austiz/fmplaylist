...
// Runtime-mutable carrier state. Updated either by the main loop's hop
// scheduler or by the control pipe's FREQ/HOP handlers; read once per
// sample by the DMA-fill loop. 32-bit aligned stores are atomic on all
// Pi targets, so no mutex is needed.
static volatile uint32_t g_freq_ctl;
static volatile uint32_t g_carrier_freq;

// Hop schedule. hop_count==0 means hopping is disabled.
#define HOP_MAX 64
static volatile uint32_t g_hop_freqs[HOP_MAX];
static volatile int g_hop_count = 0;
static volatile int g_hop_dwell_ms = 0;
static volatile int g_hop_dirty = 0;  // set by control pipe to request re-init

void set_carrier_freq_hz(uint32_t hz) {
    if (hz < 76000000 || hz > 108000000) return;
    double divider = (double)PLLFREQ / (double)hz;
    uint32_t fc = (uint32_t)(divider * (1 << 12));
    g_carrier_freq = hz;
    g_freq_ctl = fc;
}

// Parse "f1,f2,...:dwell_ms" into the hop table. Returns 1 on success.
int set_hop_schedule(const char *spec) {
    if (!spec || !*spec) return 0;
    const char *colon = strrchr(spec, ':');
    if (!colon) return 0;
    int dwell = atoi(colon + 1);
    if (dwell <= 0) return 0;

    uint32_t tmp[HOP_MAX];
    int n = 0;
    const char *p = spec;
    while (p < colon && n < HOP_MAX) {
        char *end = NULL;
        double mhz = strtod(p, &end);
        if (end == p) return 0;
        uint32_t hz = (uint32_t)(mhz * 1e6);
        if (hz < 76000000 || hz > 108000000) return 0;
        tmp[n++] = hz;
        p = end;
        while (p < colon && (*p == ',' || *p == ' ')) p++;
    }
    ...
    for (int i = 0; i < n; i++) g_hop_freqs[i] = tmp[i];
    g_hop_count = n;
    g_hop_dwell_ms = dwell;
    g_hop_dirty = 1;
    return 1;
}

void disable_hop_schedule(void) {
    g_hop_count = 0;
}
...
int tx(uint32_t carrier_freq, char *audio_file, uint16_t pi, char *ps, char *rt, float ppm, char *control_pipe, char *hop_spec) {
    ...
    // Calculate the frequency control word and seed the runtime-mutable
    // carrier state. The fractional part is stored in the lower 12 bits.
    // From here on, the sample-fill loop reads g_freq_ctl every iteration,
    // so retuning is just "store a new value into g_freq_ctl."
    set_carrier_freq_hz(carrier_freq);


    for (int i = 0; i < NUM_SAMPLES; i++) {
        ctl->sample[i] = 0x5a << 24 | g_freq_ctl;    // Silence
    ...
    // Seed the hop schedule if one was given on the CLI.
    if (hop_spec) {
        if (!set_hop_schedule(hop_spec)) {
            fprintf(stderr, "Invalid -hop spec: %s\n", hop_spec);
        } else {
            printf("Hop schedule: %d channels, %d ms dwell.\n",
                   g_hop_count, g_hop_dwell_ms);
        }
    }

    // Hop scheduler state. clock_gettime(CLOCK_MONOTONIC) is monotonic
    // across the run; we compare ms-deltas to dwell.
    struct timespec hop_t0;
    clock_gettime(CLOCK_MONOTONIC, &hop_t0);
    int hop_index = 0;

    for (;;) {
        // Channel hop. Runs at the existing ~5ms cadence below, so the
        // shortest practical dwell is ~5ms; below that, you'd hop every
        // poll. g_hop_dirty lets the control pipe restart the cycle.
        if (g_hop_count > 0) {
            if (g_hop_dirty) {
                hop_index = 0;
                clock_gettime(CLOCK_MONOTONIC, &hop_t0);
                set_carrier_freq_hz(g_hop_freqs[0]);
                g_hop_dirty = 0;
            } else {
                struct timespec now;
                clock_gettime(CLOCK_MONOTONIC, &now);
                long elapsed_ms = (now.tv_sec - hop_t0.tv_sec) * 1000L
                                + (now.tv_nsec - hop_t0.tv_nsec) / 1000000L;
                if (elapsed_ms >= g_hop_dwell_ms) {
                    hop_index = (hop_index + 1) % g_hop_count;
                    set_carrier_freq_hz(g_hop_freqs[hop_index]);
                    hop_t0 = now;
                }
            }
        }
        ...
        while (free_slots >= SUBSIZE) {
            ...
            ctl->sample[last_sample++] = (0x5A << 24 | g_freq_ctl) + intval; //(frac > j ? intval + 1 : intval);
...
int main(int argc, char **argv) {
    ...
    for(int i=1; i<argc; i++) {
        ...
        if((strcmp("-wav", arg)==0 || strcmp("-audio", arg)==0) && param != NULL) {
            ...
            i++;
            control_pipe = param;
        } else if(strcmp("-hop", arg)==0 && param != NULL) {
            i++;
            hop_spec = param;
        } else {
    ...
    int errcode = tx(carrier_freq, audio_file, pi, ps, rt, ppm, control_pipe, hop_spec);
